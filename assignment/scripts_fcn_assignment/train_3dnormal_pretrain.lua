...
opt = lapp[[
  -s,--save          (default "/home/yu/seg_proj/FCN/checkpoint")      subdirectory to save logs
...
if opt.network == '' then
  ...
  -- hint3: you might need to skip the dropout and softmax layer in the model done
     ...
     model_FCN:remove(19)
     model_FCN:remove(16)
     model_FCN:add(nn.SpatialConvolution(4096, 40, 1, 1, 1, 1))
     model_FCN:add(nn.ReArrange())
     model_FCN:add(nn.LogSoftMax())
     model_FCN:remove(1)
     model_FCN:insert(nn.SpatialConvolution(3, 96, 11, 11, 4, 4, 100, 100), 1)
...
parameters, gradParametes = model_FCN:getParameters()
