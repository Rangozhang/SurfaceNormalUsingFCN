...
list_file = '/home/yu/seg_proj/FCN/3d/list/trainlist_rand.txt'
path_dataset = '/home/yu/seg_proj/FCN/3d'
   ...
   local filename
    ...
    for i = 1, length do 
      ...
      for str in string.gmatch(list, "%S+") do
        ...
        cnt = cnt + 1
        if cnt == 1 then 
          labelname = str    -- first part is label, surface normal
        elseif cnt == 2 then 
          filename = str     -- second part is data, image
        end
...
function dataset:getByRand( )
   local idx = torch.random(1, (#(self.imagePath))[1] )             -- pretty beautiful random strategy
